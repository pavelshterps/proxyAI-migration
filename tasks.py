...
def preload_on_startup(**kwargs):
    ...
    if _HF_AVAILABLE:
        ...
        try:
            # warm up using preview-length transcription
...
def probe_audio(src: Path) -> dict:
    """
    Single ffprobe call: returns duration and first audio stream info.
    """
    cmd = [
        "ffprobe", "-v", "error",
        "-print_format", "json",
...
def prepare_wav(upload_id: str) -> (Path, float):
    """
    Ensure mono 16k WAV; returns (wav_path, duration_s).
    """
...
def convert_to_wav_and_preview(self, upload_id, correlation_id):
    """
    Prepare WAV; if short, skip preview and enqueue full transcription.
    """
    ...
    try:
        ...
        logger.error(f"[{datetime.utcnow().isoformat()}] [{cid}] PREPARE failed", exc_info=True)
        r.publish(f"progress:{upload_id}", json.dumps({"status": "error", "error": str(e)}))
        return

    logger.info(f"[{datetime.utcnow().isoformat()}] [{cid}] duration={duration:.1f}s")

    # short audio: go straight to full transcription
...
def preview_transcribe(self, upload_id, correlation_id):
    """
    Fast preview: use WhisperModel.transcribe with max_initial_timestamp.
    """
    cid = correlation_id or "?"
    ...
    if not wav_path.exists():
        logger.error(f"[{datetime.utcnow().isoformat()}] [{cid}] WAV not found for preview")
        r.publish(f"progress:{upload_id}", json.dumps({"status": "error", "error": "WAV not found"}))
        return

    # use Whisper's built-in initial clipping for preview
    model = (
...
def transcribe_segments(self, upload_id, correlation_id):
    """
    Full transcription, chunked for long audio.
    """
    cid = correlation_id or "?"
    ...
    wav_path = Path(settings.UPLOAD_FOLDER) / f"{upload_id}.wav"
    if not wav_path.exists():
        logger.error(f"[{datetime.utcnow().isoformat()}] [{cid}] WAV not found for transcription")
        r.publish(f"progress:{upload_id}", json.dumps({"status": "error", "error": "WAV not found"}))
        return
    ...
    (out_dir / "transcript.json").write_text(
        json.dumps([{"start": s.start, "end": s.end, "text": s.text} for s in all_segs],
...
def diarize_full(self, upload_id, correlation_id):
    """
    Speaker diarization: VAD + clustering on GPU.
    """
    cid = correlation_id or "?"
    ...
    if not _PN_AVAILABLE or not settings.WHISPER_DEVICE.lower().startswith("cuda"):
        logger.error(f"[{datetime.utcnow().isoformat()}] [{cid}] pyannote unavailable or not on CUDA, skipping")
        return
