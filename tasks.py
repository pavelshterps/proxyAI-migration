...
# --- Speaker stitching / embedding flags & holders ---
...
_speaker_embedding_model: Any = None  # lazy-loaded
...
def group_into_sentences(segments: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
    ...
    for seg in segments:
        ...
        if buf["end"] is not None and seg["start"] - buf["end"] > SILENCE_GAP_S:
            flush()
            buf["start"], buf["speaker"] = seg["start"], seg.get("speaker")
        buf["end"], buf["text"].append(txt) = seg["end"], buf["text"] + [txt]
...
def get_whisper_model(model_override: str = None):
    ...
    if _whisper_model is None:
        ...
        try:
            ...
            logger.info(f"[WHISPER] loaded {path} on {device} ({compute})")
...
def get_diarization_pipeline():
    ...
    if _diarization_pipeline is None:
        model_id = getattr(settings, "PYANNOTE_PIPELINE",
...
def global_cluster_speakers(raw: List[Dict[str, Any]], wav: Path, upload_id: str) -> List[Dict[str, Any]]:
    ...
    if sr != 16000:
        waveform = Resample(sr, 16000)(waveform); sr = 16000
    ...
    labels = AgglomerativeClustering(
        ...
        distance_threshold=thresh).fit_predict(X)

    logger.info(f"[{upload_id}] global clustering of {len(raw)} segments")
    for seg, lbl in zip(raw, labels):
        seg["speaker"] = f"spk_{lbl}"
    return raw


# Слияние для API (не трогаем — фронт на него смотрит)
def merge_speakers(
    ...
    def nearest(idx: int, t0: float, t1: float):
        ...
        db, da = max(0,t0-b["end"]), max(0,a["start"]-t1)
        return b if db<=da else a

    out = []
    for t in sorted(transcript, key=lambda t:t["start"]):
        t0, t1 = t["start"]-pad, t["end"]+pad
        i = bisect_left(starts, t1)
        cands = [d for d in diar_sorted[max(i-8,0):i+8] if not (d["end"]<=t0 or d["start"]>=t1)]
...
def preload_on_startup(**kwargs):
    ...
    if _HF_AVAILABLE:
        get_whisper_model()
    # грузим pyannote лишь в GPU-воркерах, CPU-воркеры не тянут память
    if _PN_AVAILABLE and ("transcribe_gpu" in argv or "diarize_gpu" in argv):
...
def transcribe_segments(self, upload_id, correlation_id):
    ...
    offset, idx = 0.0, 0
    raw_segs: List[Any] = []
    def _transcribe_with_vad(src, offs=0.0):
        ...
        for s in segs:
            s.start += offs; s.end += offs
        return segs

    while offset < duration:
        length = min(settings.CHUNK_LENGTH_S, duration-offset)
        if total_chunks>1:
            logger.info(f"[{upload_id}] transcription chunk {idx+1}/{total_chunks}: "
...
def diarize_full(self, upload_id, correlation_id):
    ...
    raw_limit = int(getattr(settings, "DIARIZATION_CHUNK_LENGTH_S", 0))
    using = bool(raw_limit and duration>raw_limit)
    total_chunks = math.ceil(duration/raw_limit) if using else 1
    ...
    while offset < duration:
        length = min(raw_limit, duration-offset)
        if using:
            logger.info(f"[{upload_id}] diarization chunk {idx+1}/{total_chunks}: "
                        f"{offset:.1f}s→{offset+length:.1f}s")
        ...
        subprocess.run([
            ...
            "-ss",str(offset),"-t",str(length),"-i",str(wav),str(tmp)
        ], check=True, stderr=subprocess.DEVNULL, stdout=subprocess.DEVNULL)

        ann = pipeline(str(tmp))
        for s,_,spk in ann.itertracks(yield_label=True):
            raw.append({"start":float(s.start)+offset,
                        "end":float(s.end)+offset,"speaker":spk})
        tmp.unlink(missing_ok=True)
        offset += length; idx += 1

    raw.sort(key=lambda x:x["start"])
    if SPEAKER_STITCH_ENABLED and using:
        raw = global_cluster_speakers(raw, wav, upload_id)
    else:
        logger.debug(f"[{upload_id}] skipping global cluster (using={using})")
    ...
    for seg in raw:
        if buf and buf["speaker"]==seg["speaker"] and seg["start"]-buf["end"]<0.1:
            ...
            if buf: diar_out.append(buf)
            buf = dict(seg)
    if buf: diar_out.append(buf)

    out = Path(settings.RESULTS_FOLDER)/upload_id
    out.mkdir(parents=True, exist_ok=True)
    (out/"diarization.json").write_text(json.dumps(diar_out, ensure_ascii=False, indent=2))
    send_webhook_event("diarization_completed", upload_id, {"diarization": diar_out})
