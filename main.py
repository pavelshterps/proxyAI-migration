...
from tasks import download_audio, preview_transcribe, diarize_full  # transcribe_segments теперь вызывается внутри preview_transcribe

@asynccontextmanager
async def lifespan(app: FastAPI):
    """Повторяем init_models при старте, если не вышло."""
    for _ in range(5):
        try:
...
async def upload(
    ...
    1) Сохранить файл
    2) Запись в БД
    3) download_audio + preview_transcribe
       (full транскрипция стартует из preview_transcribe)
    4) Инициализация прогресса в Redis
    """
