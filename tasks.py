...
_WHISPER_BATCH = settings.WHISPER_BATCH_SIZE
    ...
    for attempt in range(1, 6):
        try:
            ...
            if 400 <= code < 500:
                ...
                return
            logger.warning(f"[WEBHOOK] {event_type} returned {code}, retrying")
        except Exception as e:
            logger.warning(f"[WEBHOOK] network error attempt {attempt}: {e}")
        time.sleep(30)
...
def probe_audio(src: Path) -> dict:
    ...
    try:
        ...
        info["duration"] = float(j["format"].get("duration",0) or 0)
...
def prepare_preview_segment(upload_id: str) -> subprocess.Popen:
    ...
    return subprocess.Popen([
        ...
        "-ss","0","-t",str(settings.PREVIEW_LENGTH_S),"-i",str(src),
        "-acodec","pcm_s16le","-ac","1","-ar","16000",
        "-f","wav","pipe:1"
    ], stdout=subprocess.PIPE, stderr=subprocess.DEVNULL)

def group_into_sentences(segments: List[Dict[str,Any]]) -> List[Dict[str,Any]]:
    """Группировка по пунктуации, паузам, длительности и числу слов."""
    ...
    for seg in segments:
        ...
        if buf["end"] is not None and seg["start"]-buf["end"]>settings.SENTENCE_MAX_GAP_S:
            flush()
            buf["start"], buf["speaker"] = seg["start"], seg.get("speaker")
        buf["end"] = seg["end"]; buf["text"].append(txt)
        wc = sum(len(t.split()) for t in buf["text"])
        dur = buf["end"]-buf["start"]
        if _SENT_END_RE.search(txt) or wc>=settings.SENTENCE_MAX_WORDS or dur>=settings.SENTENCE_MAX_DURATION_S:
            flush()
    flush()
    return sentences

def merge_speakers(transcript: List[Dict], diar: List[Dict], pad: float=0.2) -> List[Dict]:
    if not diar:
        return [{**t,"speaker":None} for t in transcript]
    diar=sorted(diar,key=lambda d:d["start"]); transcript=sorted(transcript,key=lambda t:t["start"])
    starts=[d["start"] for d in diar]
    from bisect import bisect_left
    def nearest(i,t0,t1):
        if i<=0: return diar[0]
        if i>=len(diar): return diar[-1]
        b,a=diar[i-1],diar[i]; db=max(0,t0-b["end"]); da=max(0,a["start"]-t1)
        return b if db<=da else a
    out=[]
    for t in transcript:
        t0=max(0,t["start"]-pad); t1=t["end"]+pad
        i=bisect_left(starts,t1)
        cands=[d for d in diar[max(0,i-8):i+8] if not(d["end"]<=t0 or d["start"]>=t1)]
        best=max(cands,key=lambda d:max(0,min(d["end"],t1)-max(d["start"],t0))) if cands else nearest(i,t0,t1)
...
def get_whisper_model(model_override: str=None):
    ...
    if _whisper_model is None:
        ...
        try:
            ...
            path=model_id
        if device=="cpu" and compute in("fp16","float16"): compute="int8"
...
def preload_on_startup(**_):
    if _HF_AVAILABLE: get_whisper_model()
    if _PN_AVAILABLE: get_diarization_pipeline()
...
def convert_to_wav_and_preview(self, upload_id, correlation_id):
    _redis.publish(f"progress:{upload_id}", json.dumps({"status":"processing_started"}))
    deliver_webhook.delay("processing_started",upload_id,None)
    try:
        prepare_wav(upload_id)
    except Exception as e:
        _redis.publish(f"progress:{upload_id}", json.dumps({"status":"error","error":str(e)}))
...
def preview_transcribe(self, upload_id, correlation_id):
    ...
    segs,_ = model.transcribe(proc.stdout, word_timestamps=True, batch_size=_WHISPER_BATCH)
    proc.stdout.close(); proc.wait()
    segments = list(segs)
    for s in segments:
        _redis_inst.publish(f"progress:{upload_id}",
            json.dumps({"status":"preview_partial","fragment":{"start":s.start,"end":s.end,"text":s.text}})
        )
    preview = {"text":"".join(s.text for s in segments),
    ...
    _redis_inst.publish(f"progress:{upload_id}", json.dumps({"status":"preview_done","preview":preview}))
...
def transcribe_segments(self, upload_id, correlation_id):
    ...
    if duration <= settings.VAD_MAX_LENGTH_S:
        logger.info(f"[transcribe_segments] short audio {duration:.1f}s")
        segs,_ = model.transcribe(
            str(wav), word_timestamps=True,
            vad_filter=True, vad_parameters=_VAD_PARAMS,
            batch_size=_WHISPER_BATCH
        )
        raw_segs=list(segs)
    else:
        logger.info(f"[transcribe_segments] chunking at {settings.CHUNK_LENGTH_S}s")
        offset=0.0
    ...
    flat=[{"start":s.start,"end":s.end,"speaker":None,"text":s.text} for s in raw_segs]
    flat.sort(key=lambda x:x["start"])
    sentences=group_into_sentences(flat)

    out=Path(settings.RESULTS_FOLDER)/upload_id; out.mkdir(parents=True,exist_ok=True)
    (out/"transcript.json").write_text(json.dumps(sentences,ensure_ascii=False,indent=2))
    _redis_inst.publish(f"progress:{upload_id}",json.dumps({"status":"transcript_done"}))
...
def diarize_full(self, upload_id, correlation_id):
    _redis_inst = _redis
    _redis_inst.publish(f"progress:{upload_id}", json.dumps({"status":"diarize_started"}))
    ...
    _redis_inst.publish(f"progress:{upload_id}", json.dumps({"status":"diarization_done"}))
    deliver_webhook.delay("diarization_completed",upload_id,{"diarization":diar})

@app.task(bind=True,name="deliver_webhook",queue="webhooks",max_retries=5,default_retry_delay=30)
