...
def send_webhook_event(event_type: str, upload_id: str, data: Optional[Any]):
    """Publish webhook to external URL with retries."""
    ...
    for attempt in range(1, max_attempts + 1):
        try:
            ...
            logger.warning(f"[WEBHOOK] network error on attempt {attempt}/{max_attempts} for {upload_id}: {e}")
        time.sleep(30)
    logger.error(f"[WEBHOOK] {event_type} failed after {max_attempts} attempts for {upload_id}")

def probe_audio(src: Path) -> dict:
    """Inspect audio file with ffprobe."""
    res = subprocess.run(
...
def prepare_preview_segment(upload_id: str) -> subprocess.Popen:
    """Produce a short WAV preview via ffmpeg."""
    src = next(Path(settings.UPLOAD_FOLDER).glob(f"{upload_id}.*"))
...
def group_into_sentences(segments: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
    """Merge contiguous Whisper segments into sentences."""
...
def merge_speakers(
    ...
    """Assign each transcript sentence to the nearest speaker segment."""
    if not diar:
    ...
    for t in transcript:
        ...
        if cands:
            best = max(cands, key=lambda d: min(d["end"], t1) - max(d["start"], t0))
        else:
...
def get_diarization_pipeline():
    """Load or return cached pyannote.audio pipeline."""
...
def get_speaker_embedding_model():
    """Load or return cached speechbrain speaker embedding model."""
...
def stitch_speakers(raw: List[Dict[str, Any]], wav: Path, upload_id: str) -> List[Dict[str, Any]]:
    """Refine speaker labels by clustering embeddings."""
    if not SPEAKER_STITCH_ENABLED:
        return raw
    unique = set(seg.get("speaker") for seg in raw)
    if len(unique) <= 1:
        logger.debug(f"[{upload_id}] one speaker only, skipping stitching")
        return raw
    try:
        ...
        stitch_centroids, stitch_histories = {}, {}
        next_label_idx = 0
        def new_label():
            nonlocal next_label_idx
            lbl = f"spk_{next_label_idx}"
            next_label_idx += 1
            return lbl
        ...
        for seg in sorted(raw, key=lambda x: x["start"]):
            ...
            if end_s <= start_s or start_s >= waveform.size(1):
            ...
            # assign to existing or new
            best_lbl, best_sim = None, -1.0
            for lbl, cent in stitch_centroids.items():
                sim = torch.dot(emb, cent).item()
                if sim > best_sim:
                    best_sim, best_lbl = sim, lbl
            if best_lbl and best_sim >= SPEAKER_STITCH_THRESHOLD:
                # update centroid
                stitch_centroids[best_lbl] = F.normalize(
                    SPEAKER_STITCH_EMA_ALPHA * emb + (1 - SPEAKER_STITCH_EMA_ALPHA) * stitch_centroids[best_lbl],
                    p=2, dim=0
                )
                stitch_histories[best_lbl].append(emb)
                if len(stitch_histories[best_lbl]) > SPEAKER_STITCH_POOL_SIZE:
                    stitch_histories[best_lbl].pop(0)
                seg["speaker"] = best_lbl
            else:
                new_lbl = new_label()
                stitch_centroids[new_lbl], stitch_histories[new_lbl] = emb, [emb]
                seg["speaker"] = new_lbl
            stitched.append(seg)
        # optional merge of similar speakers
        # ... (same as above) ...
        return stitched
    except Exception as e:
        logger.warning(f"[{upload_id}] speaker stitching error: {e}")
        return raw
...
def preload_on_startup(**kwargs):
    ...
    # limit per-process CUDA memory
    try:
        import torch
        if torch.cuda.is_available():
            torch.cuda.set_per_process_memory_fraction(0.5, device=0)
            logger.info("[INIT] CUDA memory fraction limited to 50%")
    except ImportError:
        pass
...
def preview_transcribe(self, upload_id, correlation_id):
    ...
    # fallback check omitted for brevity
    r = Redis.from_url(settings.CELERY_BROKER_URL, decode_responses=True)
    ...
    def _transcribe_with_vad(source, offset=0.0):
        try:
            ...
            if "out of memory" in str(e).lower():
                ...
                logger.error(f"[{upload_id}] CUDA OOM during transcription, retrying")
                time.sleep(5)
                return _transcribe_with_vad(source, offset)
            else:
                raise
...
def diarize_full(self, upload_id, correlation_id):
    ...
    try:
        ...
        logger.warning(f"[{upload_id}] invalid chunk limit, disabling chunking")
        chunk_limit = 0

    using_chunking = bool(chunk_limit and duration > chunk_limit)
    total = math.ceil(duration / chunk_limit) if using_chunking else 1
    logger.info(f"[{upload_id}] using_chunking={using_chunking}; total_chunks={total}")

    if not _PN_AVAILABLE:
        logger.error(f"[{upload_id}] pyannote.audio unavailable")
        send_webhook_event("processing_failed", upload_id, None)
        return
    ...
    if using_chunking:
        ...
        while offset < duration:
            ...
            try:
                ...
                if "out of memory" in str(e).lower():
                    import torch; torch.cuda.empty_cache()
                    logger.error(f"[{upload_id}] CUDA OOM in diarize chunk {idx+1}, retrying")
                    time.sleep(5)
                    ann = pipeline(str(tmp))
                else:
                    raise
        ...
        try:
            ...
            if "out of memory" in str(e).lower():
                import torch; torch.cuda.empty_cache()
                logger.error(f"[{upload_id}] CUDA OOM in single-pass diarization, retrying")
                time.sleep(5)
                ann = pipeline(str(wav))
            else:
                raise
    ...
    # Stitch speakers if enabled and chunked
...
def deliver_webhook(self, event_type: str, upload_id: str, data: Optional[Any]):
    """Celery-delivered webhook task with retries."""
