        ...
        for node_tasks in active.values():
            for t in node_tasks:
                if t["name"] in ("tasks.diarize_full", "tasks.transcribe_segments"):
                    heavy += 1
        if heavy >= 2:
            logger.info(f"[{upload_id}] both GPUs busy, falling back to CPU preview")
    ...
    if duration <= settings.VAD_MAX_LENGTH_S:
        try:
            ...
            if "out of memory" in str(e).lower():
                ...
                logger.error(f"[{upload_id}] CUDA OOM, retry after delay")
                time.sleep(5)
                raw_segs = _transcribe_with_vad(str(wav))
            else:
                raise
...
def diarize_full(self, upload_id, correlation_id):
    ...
    logger.info(f"[{upload_id}] using_chunking={using_chunking}; total_chunks={total_chunks}")
    ...
    if using_chunking:
        ...
        while offset < duration:
            ...
            try:
                ...
                if "out of memory" in str(e).lower():
                    import torch; torch.cuda.empty_cache()
                    logger.error(f"[{upload_id}] CUDA OOM in diarize chunk {chunk_idx+1}, retry")
                    time.sleep(5)
                    ann = pipeline(str(tmp))
                else:
                    raise
        ...
        try:
            ...
            if "out of memory" in str(e).lower():
                import torch; torch.cuda.empty_cache()
                logger.error(f"[{upload_id}] CUDA OOM in single-pass diarization, retry")
                time.sleep(5)
                ann = pipeline(str(wav))
            else:
                raise
    ...
    # … здесь остаётся остальной код по стичу и сохранению diarization.json …
