...
def send_webhook_event(event_type: str, upload_id: str, data: Optional[Any]):
    """
    Отправка webhook-события с повтором в случае ошибки:
      • если впервые 2xx — больше не шлём;
      • иначе — логируем и ждём следующей попытки (через 30s при новом вызове).
    """
    ...
    try:
        ...
        if 200 <= resp.status_code < 300:
            ...
            logger.warning(f"[{datetime.utcnow().isoformat()}] [WEBHOOK] non-2xx ({resp.status_code}) for {upload_id}/{event_type}")
    except Exception as e:
        logger.warning(f"[{datetime.utcnow().isoformat()}] [WEBHOOK] failed {event_type} for {upload_id}: {e}")

# Флаги наличия моделей
try:
