...
def send_webhook_event(event_type: str, upload_id: str, data: Optional[Any]):
    """
    Отправляет вебхук один раз (если вернулся 2xx, больше не шлём).
    При ошибке или не‐2xx — просто логируем, и можно будет повторить через 30s.
    """
    ...
    try:
        ...
        if 200 <= resp.status_code < 300:
            ...
            logger.info(f"[WEBHOOK] {event_type} for {upload_id} sent, status {resp.status_code}")
        else:
            logger.warning(f"[WEBHOOK] {event_type} for {upload_id} returned {resp.status_code}")
    except Exception as e:
        logger.warning(f"[WEBHOOK] {event_type} for {upload_id} failed: {e}")
...
def get_whisper_model(model_override: str = None):
    ...
    if model_override:
        logger.info(f"[WHISPER] override model {model_override} on {device} ({compute})")
        return WhisperModel(model_override, device=device, compute_type=compute)

    global _whisper_model
    if _whisper_model is None:
        model_id = settings.WHISPER_MODEL_PATH
        logger.info(f"[WHISPER] loading model {model_id} on {device} ({compute})")
        try:
            path = download_model(
                model_id,
...
def prepare_wav(upload_id: str) -> (Path, float):
    ...
    if (src.suffix.lower() == ".wav"
        ...
        if src != target:
            src.rename(target)
        logger.info(f"[PREPARE] {upload_id} WAV OK ({time.perf_counter()-start:.2f}s)")
    ...
    logger.info(f"[PREPARE] {upload_id} converted ({time.perf_counter()-start:.2f}s)")
    return target, duration
...
def convert_to_wav_and_preview(self, upload_id, correlation_id):
    ...
    try:
        logger.info(f"[{cid}] convert_to_wav_and_preview start: {upload_id}")
        ...
        return
    logger.info(f"[{cid}] enqueue preview_transcribe")
...
def preview_transcribe(self, upload_id, correlation_id):
    ...
    r = Redis.from_url(settings.CELERY_BROKER_URL, decode_responses=True)
    try:
        logger.info(f"[{cid}] preview_transcribe start: {upload_id}")
...
def transcribe_segments(self, upload_id, correlation_id):
    ...
    r = Redis.from_url(settings.CELERY_BROKER_URL, decode_responses=True)
    try:
        logger.info(f"[{cid}] transcribe_segments start: {upload_id}")
        wav = Path(settings.UPLOAD_FOLDER) / f"{upload_id}.wav"
