...
def get_whisper_model(model_override: str = None):
    """
    Return a WhisperModel instance, optionally overriding with a lighter model for preview.
    """
    ...
    if _whisper_model is None:
        logger.info(f"[{datetime.utcnow().isoformat()}] [WHISPER] initializing model {settings.WHISPER_MODEL_PATH} on {device} ({compute})")
...
def get_vad():
    """
    Lazy-load and return the VoiceActivityDetection pipeline.
    """
    global _vad
    if _vad is None:
...
def get_clustering_diarizer():
    """
    Lazy-load and return the SpeakerDiarization pipeline.
    """
...
def preload_on_startup(**kwargs):
    """
    Warm up models on worker start to avoid first-request delays.
    """
    device = settings.WHISPER_DEVICE.lower()
...
def prepare_wav(upload_id: str) -> Path:
    """
    Convert any upload_id.* to upload_id.wav (pcm_s16le@16k mono) if needed.
    Otherwise rename.
    """
    ...
    try:
        ...
        if (src.suffix.lower() == ".wav" and
            ...
            if src != target:
                src.rename(target)
            logger.info(f"[{datetime.utcnow().isoformat()}] [PREPARE] already OK, renamed to {target.name}")
            return target
    except Exception:
        pass
    ...
    logger.info(f"[{datetime.utcnow().isoformat()}] [PREPARE] converted to {target.name}")
    return target
...
def convert_to_wav_and_preview(self, upload_id, correlation_id):
    """
    Task: prepare WAV and enqueue preview transcription.
    """
    ...
    logger.info(f"[{datetime.utcnow().isoformat()}] [{cid}] WAV ready ({wav.name}), enqueue PREVIEW")
...
def preview_transcribe(self, upload_id, correlation_id):
    """
    Fast preview: pipe first N seconds through ffmpeg into Whisper.
    """
    cid = correlation_id or "?"
    ...
    # Pipe ffmpeg for first PREVIEW_LENGTH_S seconds
    threads = getattr(settings, "FFMPEG_THREADS", 2)
    cmd = [
        "ffmpeg", "-y", "-threads", str(threads),
        "-i", str(wav_path),
    ...
    try:
        logger.info(f"[{datetime.utcnow().isoformat()}] [{cid}] spawning ffmpeg pipe")
        ...
        logger.info(f"[{datetime.utcnow().isoformat()}] [{cid}] got {len(segments)} preview segments")
        for seg in segments:
            r.publish(
...
def transcribe_segments(self, upload_id, correlation_id):
    """
    Full transcription of the entire WAV file.
    """
    cid = correlation_id or "?"
    ...
    try:
        logger.info(f"[{datetime.utcnow().isoformat()}] [{cid}] calling WhisperModel.transcribe")
        ...
        segments = list(segments_gen)
        logger.info(f"[{datetime.utcnow().isoformat()}] [{cid}] got {len(segments)} full segments")
    except Exception as e:
...
def diarize_full(self, upload_id, correlation_id):
    """
    Speaker diarization: VAD + clustering diarizer on GPU.
    """
    cid = correlation_id or "?"
...
def cleanup_old_files(self):
    """
    Periodic cleanup of old files beyond retention period.
    """
